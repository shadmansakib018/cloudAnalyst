...
//		System.out.println("data center numbers in default settings: "+dataCenters.size());
	...
	public void runSimulation() throws Exception {
		...
		for (DataCenterUIElement d : dataCenters) {
			...
			if (d.isAllocated()){
				...
				vmlist = createVM(brokerId, d.getVmAllocation().getVmCount(), d.getVmAllocation());
		...
		System.out.println("************Simultaion time: "+duration );
	...
	private void printVmAllocations(String dcName, Map<Integer, Integer> list){
		...
		System.out.println(loadBalancePolicy);
		for (Integer vm : list.keySet()){
//			if(vm == -1 && loadBalancePolicy == "Equal Utilization") {
//				continue;
//			}
	...
	private VirtualMachineList createVM(int userID, int vms, VmAllocationUIElement characteristicsArr) {
		System.out.println("creating VM convig: simulation file line:323");
		// Creates a container to store VMs. This list is passed to the broker 
		// later
		VirtualMachineList list = new VirtualMachineList();

		//VM Parameters
		long size = characteristicsArr.getImageSize(); //image size (MB)
		int memory = characteristicsArr.getMemory(); //vm memory (MB)
		long bw = characteristicsArr.getBw();
		...
		for (int i = vms / 2; i < vms; i++) {
			vm[i] = new VirtualMachine(new VMCharacteristics(i, userID, size/2,
					memory/2, bw/2, vcpus, priority, vmm,
					new TimeSharedVMScheduler()));
