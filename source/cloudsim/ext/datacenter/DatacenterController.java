...
public class DatacenterController extends DatacenterBroker implements GeoLocatable,
	...
	public DatacenterController(String name, 
		...
		System.out.println(requestsPerCloudlet);
		...
		else if (loadBalancePolicy.equals(Constants.LOAD_BALANCE_DYNAMIC)){
			System.out.println("dynamic LB");

			this.loadBalancer = new DynamicVmLoadBalancer(this);
		} 
//		else if (loadBalancePolicy.equals(Constants.LOAD_BALANCE_THRESHOLD)){
//			System.out.println("Single");
//			//this.loadBalancer =new SingleThresholdVmLoadBalancer(this);
...
//		} 
	...
	public VirtualMachineList get_VMlist() {
		return vmlist;
...
//			System.out.println("Processing time: "+thisProcessingTime);
	...
	private void cloudletCompletedProcessing(InternetCloudlet cl){
		...
		e.addParameter("DATA_SIZE", cl.getDataSize());
	...
	private void submitWaitingCloudlet(){
		if(waitingQueue.size() > 0){
			...
			int nextAvailVM = loadBalancer.getNextAvailableVm(vmlist, cl);
			if (nextAvailVM == -1){
...
//		}
	...
	private void submitNewCloudlet(InternetCloudlet cl) {
		...
		int nextAvailVM = loadBalancer.getNextAvailableVm(vmlist, cl);
//		System.out.println("cl request count "+cl.getRequestCount());
//		System.out.println("************************* vm chosen: "+nextAvailVM);
//		System.out.println("waitingQueue");
		if (nextAvailVM == -1){
			//All VM's are busy. Put it in queue
			//System.out.println("VM's busy, queueing " + cl);
...
//		System.out.println(GridSim.clock()+": "+this.get_name()+ ": Sending cloudlet "+cl.getGridletID()+" to VM #"+((VirtualMachine)vmlist.get(vmId)).getVmId() + ", dest=" + dest);
		super.send(dest,GridSimTags.SCHEDULE_NOW, GridSimTags.GRIDLET_SUBMIT, cl, cl.getDataSize());
